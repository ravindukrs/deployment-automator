...
def run_jmeter():
    ...
    shellcommand = "./apache-jmeter-5.4.3/bin/jmeter -n -t ./deployment-performance/jmeter-script.jmx -l ./deployment-performance/perf-results/jtl/testresults" + str(
        datetime.now().strftime("%d-%m-%Y-%H:%M:%S")) + ".jtl | awk '/summary =/ {print $7,$16}'"
    ...
    tps = non_decimal.sub('', result[0])

    error_percentage = result[1]

    return tps, error_percentage
...
def target_function(configuration):
    ...
    if (constraints_result[0] >= 0.0 and constraints_result[1] >= 0.0):
        ...
        tps, error_percentage = run_jmeter()

        print("Average TPS of Deployment:", tps)
        ...
        print("TPS as a Float: ", float(tps))
        # For Latency
        errfloat = float(re.findall("\d+\.\d+", error_percentage)[0])
        if(errfloat > 1):
            result.append((float(tps)*(1/errfloat)) * -1)
        else:
            result.append((float(tps)) * -1)
        # # For TPS
